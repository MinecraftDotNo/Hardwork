...
public abstract class PircBot implements ReplyConstants {
    ...
    private static final int ADMIN_ADD = 3;
    private static final int ADMIN_REMOVE = 4;
    private static final int OP_ADD = 5;
    private static final int OP_REMOVE = 6;
    private static final int HOP_ADD = 7;
    private static final int HOP_REMOVE = 8;
    private static final int VOICE_ADD = 9;
    private static final int VOICE_REMOVE = 10;
    ...
    public final synchronized void reconnect() throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if (i < 3) {
            ...
            throw new IrcException("reconnect to many times to the IRC server!");
        }
    }
    ...
    private final void processServerResponse(int code, String response) {
        ...
        if (code == RPL_LIST) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                String prefix = "";
                if (nick.startsWith("~")) {
                    // User is an owner in this channel.
                    prefix = "~";
                } else if (nick.startsWith("&")) {
                    // User is protected in this channel.
                    prefix = "&";
                } else if (nick.startsWith("@")) {
        ...
        if (_channelPrefixes.indexOf(target.charAt(0)) >= 0) {
            ...
            for (int i = 0; i < params[0].length(); i++) {
                ...
                if (atPos == '+' || atPos == '-') {
                    ...
                    if (pn == '+') {
                        this.updateUser(channel, ADMIN_ADD, params[p]);
                        onAdmin(channel, sourceNick, sourceLogin, sourceHostname, params[p]);
                    } else {
                        this.updateUser(channel, ADMIN_REMOVE, params[p]);
                        onDeAdmin(channel, sourceNick, sourceLogin, sourceHostname, params[p]);
                    }
                    p++;
                } else if (atPos == 'o') {
                    if (pn == '+') {
    ...
    protected void onHop(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) {
    }

    protected void onDeHop(String channel, String sourceNick, String sourceLogin, String sourceHostname, String recipient) {
    }
                ...
                while (enumeration.hasMoreElements()) {
                    ...
                    if (userObj.getNick().equalsIgnoreCase(nick)) {
                        if (userMode == OWNER_ADD) {
                            ...
                            newUser = new User(nwprefix, nick);
                            //newUser = new User();
