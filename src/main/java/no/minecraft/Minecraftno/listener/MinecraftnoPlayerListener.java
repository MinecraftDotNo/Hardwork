...
public class MinecraftnoPlayerListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.LEFT_CLICK_AIR) {
            ...
            switch (itemHand.getType()) {
                /* Only require one bonemeal for growing trees. */
                case INK_SACK: {
                    if (itemHand.getData().getData() == 15 && block.getType() == Material.SAPLING) {
                        /* Remove sapling and generate tree */
                        TreeSpecies ts = TreeSpecies.getByData(block.getData());
                        if (ts != null) {
                            ...
                            if (ts.name().equalsIgnoreCase("generic")) {
                                Random r = new Random();
                                if (r.nextInt(5) >= 2) {
                                    tt = TreeType.TREE;
                                } else {
                                ...
                                if (tt == TreeType.JUNGLE || tt == TreeType.DARK_OAK) {
                                    ...
                                    for (BlockFace f : faces) {
                                        ...
                                        if (r != null & r.getType() == Material.SAPLING && r.getData() == block.getData()) {
                                            saplingsfound++;
                                        }
                                    }

                                    if (saplingsfound > 0 && saplingsfound < 3) {
                                        return;
                                    } else if (saplingsfound == 0) {
                                        if (tt == TreeType.JUNGLE) {
                                            tt = TreeType.SMALL_JUNGLE;
                                        } else if (tt == TreeType.DARK_OAK) {
                                            return; // Dark oak dosen't have a small type.
                                        }
                                    }
                                }
                            }
